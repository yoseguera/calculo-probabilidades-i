...
% ---------------------------------------------------------
% CONFIGURACIÓN DE PÁGINA Y MÁRGENES (Tufte)
% ---------------------------------------------------------
% Márgenes Tufte: proporción 1:2:1 en horizontal
% Izquierdo: 2cm, Derecho: 4cm (para notas y anotaciones)
...
\newcommand{\norma}[1]{|| #1 ||}
\newcommand{\anulador}[1]{\prescript{0}{}{#1}}

\DeclareMathOperator{\Cov}{Cov} 

% ----------------------------------------------------
% CONFIGURACIÓN TEOREMAS
% ----------------------------------------------------
\newtheoremstyle{mystyle}%
  {\topsep}%                   espacio antes
  {\topsep}%                   espacio después
  {\itshape}%                  cuerpo
  {}%                          indentación
  {\bfseries}%                 encabezado
  {}%                         puntuación después del encabezado
  {\newline}%                  espacio después del encabezado (nueva línea)
  {\thmname{#1}\thmnumber{ #2}\thmnote{ \emph{(#3)}}}% formato personalizado
...
% ---------------------------------------------------------
% CONFIGURACIÓN DE NOTAS AL PIE COMO NOTAS AL MARGEN
% ---------------------------------------------------------
% En el preámbulo
\newcounter{sidenote}
...
\label{espacio-muest ral-y-sucesos}
...
\begin{itemize}
  ...
  \item \(P(B) = P(A) + P(B \cap A^c)\).
  \item Si \(A \subset B \subset \Omega\), entonces \(P(B - A) = P(B \cap A^c) = P(B)- P(A)\). 
...
\chapter{Las fórmulas de inclusión y exclusión}
...
En un espacio de probabilidad, si \(A_1, A_2,\ldots,A_n\) son sucesos cualesquiera, la probabilidad \(P_{[m]}\) de que ocurran exactamente \(m\) de ellos, es\footnote{$S_0 = 1$}
\[
...
\section{Aplicaciones de la probabilidad condicionada}

\subsection{Probabilidad con remplazo}

Probabilidad de obtener \(k\) bolas blancas en \(n\) extracciones con reemplazo de una urna con proporción \(p\) de bolas blancas.
\[
P\{X=k\}=\binom{n}{k} p^k (1-p)^{n-k}
\]

\subsection{Probabilidad sin remplazo}

Probabilidad de obtener \(k\) bolas blancas en \(n\) extracciones sin reemplazo de una urna con proporción \(p\) de bolas blancas.
\[
P\{X=k\}=\frac{\binom{Np}{k} \binom{N(1-p)}{n-k}}{\binom{Np}{n}}
\]
...
En un espacio de probabilidad, tres sucesos \(A_1, A_2, A_3\) son independientes si se cumplen las cuatro condiciones:
\begin{enumerate}
...
En un espacio de \emph{probabilidad discreto} \((\Omega,P)\), se denomina \emph{variable aleatoria}, a cualquier función
\[
...
La colección de probabilidades \(P\{X \in B\}\), correspondientes a cada subconjunto \(B\) de $\R$, se denomina la \emph{distribución de la variable aleatoria $X$}.
...
\section{Variables aleatorias simultáneas}
...
Dos variables aleatorias tendrán una \emph{relación funcional} cuando una vez elegida una de ellas, la segunda queda determinada sin ninguna aleatoriedad.
...
\begin{align*}
  ...
  P\{X_2 = x_2\} &= \sum_{x_1 \in X_1(\Omega)} P\{X_1 = x_1, X_2 = x_2\}
\end{align*}
...
\subsection{Rachas en $n$ lanzamientos de monedas}

\[
P\{X=r\} = \binom{n-1}{r-1}\frac{1}{2^{n-1}}
\]

\include{08-esperanza-matematica}

\include{09-analisis-descriptivo}

\include{10-pruebas-repetidas}

\include{I-combinatoria}