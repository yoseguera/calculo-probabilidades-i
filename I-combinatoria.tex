...
Una colección de \(n\) objetos, clasificados en \(k\) grupos de objetos idénticos entre sí, el primero con \(n_1\) objetos, el segundo con \(n_2\ldots\) se pueden ordenar en fila de
\[
...
Hay \[n(n-1)(n-2)\cdots(n-r+1) = \frac{n!}{(n-r)!} = (n)_r\] subconjuntos ordenados posibles de \(r\) elementos, que pueden extraerse de un conjunto de \(n\) elementos. También conocidos como \emph{variaciones sin repetición\footnote{Un elemento no puede ser elegido varias veces.} de \(r\) elementos tomados entre \(n\)}.

El número de \textbf{variaciones con repetición}\footnote{Un elemento puede ser elegido varias veces.} de \(r\) elementos tomados entre \(n\) es \(n^r\). 
...
Si hay que repartir \(r\) objetos iguales en \(n\) grupos, siendo posible que haya alguno vacío, existen 
\[
\binom{n+r-1}{r}
\] 
repartos posibles. 

Si no es posible que haya algún grupo vacío, el número de repartos posibles es
\[\binom{r-1}{n-1}\] 
siempre que \(r \geq n\); en caso contrario, el número de repartos posibles es cero.
...
\[
\sum_{k=0}^\infty (-1)^k \binom{r}{n} \binom{2n-2k}{2r-2k} = \binom{n}{2r}2^{2r}
\]

\subsection{Identidad inmediata}
...
\begin{align*}
	...
	&=\sum_{k=0}^n \binom{x}{k}\binom{y}{n-k}
\end{align*}
